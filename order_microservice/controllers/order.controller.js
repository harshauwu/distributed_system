...
const saveOrder = async (req, res) => {
    ...
    try {
        ...
        const orderDetails  = { customer_id: ObjectId(customerId), amount: amount, status: STATUS.PENDING.id, ship_name: req.body.shipping_name, shipping_address: req.body.shipping_address};
        const orderData = await orderService.saveOrder(orderDetails);
        
        if (orderData) {
            orderId = orderData._id;
            const orderItemsDetails = manipulateOrderItems(req.body.items, orderId, customerId);
...
const manipulateOrderItems = (items, orderId, customerId) => {
    let orderItems = [];
    items.map((item) => {
        orderItems.push({ order_id : ObjectId(orderId), customer_id : ObjectId(customerId), product_id : ObjectId(item.product_id), unit_price : item.unit_price, quantity : item.quantity});
    });
    return orderItems;
}
